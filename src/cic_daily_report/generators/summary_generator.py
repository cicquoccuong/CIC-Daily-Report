...
def _build_prompt(today: str, data_context: str) -> str:
    ...
    return (
        ...
        "=== QUY TẮC ===\n"
        "- Ngôn ngữ: tiếng Việt, dễ hiểu, phù hợp đọc trên điện thoại\n"
        "- KHÔNG khuyến nghị mua/bán (tuân thủ NQ05)\n"
        "- KHÔNG dùng từ 'tiền điện tử', thay bằng 'tài sản mã hóa'\n"
        "- Copy-paste ready cho Telegram group (không dùng markdown phức tạp)\n"
        "- Dùng bold (**text**) cho số liệu quan trọng\n"
        "- Mỗi phần phải dùng DỮ LIỆU THỰC từ trên, không bịa số\n"
