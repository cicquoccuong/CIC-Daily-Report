...
**Requirements revealed**: Multi-LLM fallback (Gemini 2.5 Flash → Flash-Lite → Groq → Cerebras), partial delivery support, error notification to Telegram with clear status (cái nào xong, cái nào retry, ETA), retry logic, graceful degradation.
...
|---------|-----------|-------------|----------|
| Gemini 2.5 Flash (PRIMARY) | 10 req/min | 250 req/day | → Flash-Lite → Groq |
| Groq (Qwen3-32B / Llama 4 Scout) | 30 req/min | 14,400 req/day | → Cerebras |
...
- **FR34**: Pipeline supports multi-LLM fallback (Gemini 2.5 Flash → Flash-Lite → Groq Qwen3 → Llama 4 → Cerebras)
